...
```mermaid
flowchart TD
    A["Xác định nhu cầu VL/TB<br/>(Theo tiến độ thi công)"] --> B["Tìm hiểu yêu cầu KH<br/>(Phong cách, ngân sách, ưu tiên)"]
    B --> C["Tham vấn AA (thẩm mỹ)<br/>& CA (kỹ thuật)"]
    C --> D["Đề xuất ≥ 3 phương án<br/>(Bảng so sánh VL/TB)"]
    D --> E["Trình bày cho KH<br/>(Ưu/nhược điểm từng PA)"]
    E --> F{"KH chọn?"}
    F -->|Chọn| G["Lên đơn hàng<br/>& theo dõi giao hàng"]
    F -->|Chưa ưng| H["Tìm thêm PA mới"]
    H --> D
    G --> I["Nghiệm thu VL/TB<br/>khi nhận hàng"]
```
